...
try {
  ...
  $pdo->prepare('UPDATE tests SET results_published_at=NOW(), updated_by=?, updated_at=NOW() WHERE id=?')
      ->execute([(int)$u['id'], $test_id]);
  $pdo->prepare('INSERT INTO test_audit_log (test_id, user_id, action, details) VALUES (?,?,?,?)')
      ->execute([$test_id, (int)$u['id'], 'publish_results', null]);
  json_ok(['published' => true]);
} catch (Throwable $e) {
